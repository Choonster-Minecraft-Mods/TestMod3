...
public class ShapedArmourUpgradeRecipe extends ShapedRecipe {
		...
		if (!output.isEmpty()) {
			for (int i = 0; i < inv.getContainerSize(); i++) { // For each slot in the crafting inventory,
				...
				if (!ingredient.isEmpty() && ingredient.getItem() instanceof ArmorItem) { // If it's an armour item,
					...
					final int newDamage = MathHelper.clamp(ingredient.getDamageValue(), 0, output.getMaxDamage());
	...
	public static class Serializer extends ForgeRegistryEntry<IRecipeSerializer<?>> implements IRecipeSerializer<ShapedArmourUpgradeRecipe> {
		...
		@Override
		public ShapedArmourUpgradeRecipe fromNetwork(final ResourceLocation recipeID, final PacketBuffer buffer) {
		...
		@Override
		public void toNetwork(final PacketBuffer buffer, final ShapedArmourUpgradeRecipe recipe) {
