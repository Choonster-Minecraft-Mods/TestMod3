...
import choonster.testmod3.tileentity.TileEntityFluidTankBase;
 ...
 * @author Choonster
 */
public class BlockFluidTank<TE extends TileEntityFluidTankBase> extends BlockTileEntity<TE> {
	...
	@Override
	public void onBlockPlacedBy(final World worldIn, final BlockPos pos, final IBlockState state, @Nullable final EntityLivingBase placer, final ItemStack stack) {
	...
	public boolean onBlockActivated(final IBlockState state, final World worldIn, final BlockPos pos, final EntityPlayer player, final EnumHand hand, final EnumFacing side, final float hitX, final float hitY, final float hitZ) {
		...
		return getFluidHandler(worldIn, pos)
				.map(fluidHandler -> {
					...
					if (!worldIn.isRemote && (success || hand == EnumHand.OFF_HAND)) {
						TestMod3.network.send(PacketDistributor.PLAYER.with(() -> (EntityPlayerMP) player), new MessageFluidTankContents(fluidHandler.getTankProperties()));
					}
